...
const Sidebar = (props) => {
  return (
    <aside
      className={
        props.toggle
          ? 'lg:visibility-hidden absolute top-12 left-0 w-screen h-[calc(100vh-48px)] bg-black ease-in duration-300 opacity-100 translate-y-0 transition-all'
          : 'overflow-y-hidden ease-in duration-300 h-0 opacity-0 translate-y-[-10px] transition-all'
      }
    >
